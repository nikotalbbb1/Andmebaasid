...
--
-- Host: 127.0.0.1
-- Loomise aeg: Märts 16, 2026 kell 08:58 EL
...
-- Toimingud
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `filmOtsimine` (IN `nimi` VARCHAR(10))   BEGIN
SELECT filmID, filmNimetus from film
where filmNimetus like concat(nimi,'%');
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Lisamine` (IN `zanrNimi` VARCHAR(30))   begin
insert into zanr(zanrNimi)
values (zanrNimi);
select * from zanr;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `rezisoorAlter` (IN `veerunimi` VARCHAR(25))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Uuendamine` (IN `rezID` INT, IN `uuseesnimi` VARCHAR(25), IN `uusperenimi` VARCHAR(25))   BEGIN
UPDATE rezisoor
...
--
ALTER TABLE `zanr`
  MODIFY `zanrID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=15;
